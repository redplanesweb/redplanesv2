...
const InitialPoint = ({ firebase }) => {
        ...
        firebase.firestore().collection("maps").get().then(function (querySnapshot) {
            ...
            firebase.firestore().collection('uf_to_peso').get().then(snapshot => {
                ...
                let maps = {
                    ...
                    providerMap: docs[docs.map(e => e.id).indexOf('provider_map')].data,
    ...
    return (
        <React.Fragment>
            ...
            <SearchLayout submit={submit} people={people} maps={maps} firebase={firebase} currentRate={currentRate} />
...
const SearchLayout = ({ submit, people, maps, firebase, currentRate }) => {
    ...
    React.useEffect(() => {
        ...
        firebase.firestore().collection('insurance_entries').get().then(snapshot => {
            ...
            localStorage.setItem('insurance_entries', JSON.stringify(final_payload))
    ...
    React.useEffect(() => {
        ...
        let results = allEntries.filter((entry, index) => {
            ...
            // Filter By clinic
            let clinic = entry.medical_centers.some(center => filter.clinics.includes(center.name))

            // Filter by percentages
            let hospital = entry.medical_centers.some(center => parseInt(center.hospital) >= filter.coverage.hospital)
            let ambulence = entry.medical_centers.some(center => parseInt(center.ambulence) >= filter.coverage.ambulence)
            let urgent = entry.medical_centers.some(center => parseInt(center.urgent) >= filter.coverage.urgent)
            ...
            <Container>
                <Grid container spacing={2}>
                    <Grid item xs={12} md={8}>
                        <CircularProgress color="secondary" style={{ display: entries ? 'none' : 'block', margin: '2em auto' }} />

                        {
                            entries &&
                            entries.filter(entry => parseInt(entry.price) < parseInt(filter.price_range[1]) && parseInt(entry.price) > parseInt(filter.price_range[0])).map((entry, index) => {
